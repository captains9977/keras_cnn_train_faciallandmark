...
def draw_bounding_cube(frame, coords, direction_vector):
    MAGNITUDE = 200
    """0,32"""
    PADDING = 25
    padding_factor = 0.5
    face_start_x, face_start_y = coords[0]
    face_stop_x, face_stop_y = coords[32]
    _, chin_y = coords[16]
...
def show_face(face, key_points):
    ...
    # cv2.line(frame, (0, y_points[54]),
    #          (width+PADDING, y_points[54]), (0, 0, 255), 1)
    # cv2.line(frame, (x_points[54], 0),
    #          (x_points[54], height+PADDING), (0, 0, 255), 1)
    for i, coord in enumerate(coords, start=0):
        x, y = coord
